...
%vect_disbalance = [vect_disbalance(1:29),linspace(vect_disbalance(29),vect_disbalance(15),29)];   % Проверяем обратный ход
...
%% Построение биффуркационной диаграммы
 ...
 hold on;box on; grid on;
  for j = 1:1:le_vect_disbalance
      ...
      elseif Average(j)<=0 && j>29
        plot(vect_disbalance(j),(MaxKSIx{j})','bo','MarkerSize',14)
      else
 ...
 title([num2str(nT-NUMREV),' < nT < ',num2str(nT),...
        '; \epsilon = var(',num2str(vect_disbalance(1)),', ',num2str(vect_disbalance(29)),') ',...
        'end reverse \epsilon = var(',num2str(vect_disbalance(29)),', ',num2str(vect_disbalance(15)),')' ]);
    
 %% Просмотр реализаций
 IND = 28;     % Индекс реализации
 tN = time{IND}/TN;
