...
vect_freq = linspace(0,0.12,13);
le_vect_freq = length(vect_freq);

tic
%% Получение решения
NUMREV=300;
ind = 29;
for j=1:1:le_vect_freq
    j
    %[t,Z0]=ode23t(@(t,Z0) rGap_Multiple_Supports(t,Z0,DD,A0,A1,A2,N,eD,mnew,eta_s,kappa_s,nn,D_s,vect_disbalance(ind),gd,vect_freq(j),NumbOp,ii,System_name),T,x0,opt);
    [t,Z0]=ode23t(@(t,Z0) rGap_Multiple_Supports(t,Z0,DD,A0,A1,A2,N,eD,mnew,eta_s,kappa_s,nn,D_s,e,gd,vect_freq(j),NumbOp,ii,System_name),T,x0,opt);
    tEta=t/TN;
...
ff=get(gca);
     ...
     title([num2str(nT-NUMREV),' < nT < ',num2str(nT),'; ','\epsilon = \epsilon_{*}^{+}','; ',...                  % Если "+"
        'f = ',num2str(vect_freq(indexPressigion))])
     grid on;
...
%% Построение биффуркационной диаграммы
 ...
 hold on;box on; grid on;  
  for j=1:1:le_vect_freq
      if Average(j)>=0
        plot(vect_freq(j),(MaxKSIx{j})','k.','MarkerSize',18)
      else 
        plot(vect_freq(j),(MaxKSIx{j})','r.','MarkerSize',18)
      end
  end
 ff=get(gca);
 plot(ff.XLim,[eta_s,eta_s]*0.5,'k-',ff.XLim,[0,0],'k-',ff.XLim,-[eta_s,eta_s],'k-')
 %plot(ff.XLim,[eta_s,eta_s],'k-',ff.XLim,[0,0],'k-',ff.XLim,-[eta_s,eta_s]*cos(54*pi/180),'k-')
 xlabel('f','FontName','Times New Roman','FontSize',20)
...
%         'f = var(',num2str(vect_freq(1)),',',num2str(vect_freq(end)),')'])
 title([num2str(nT-NUMREV),' < nT < ',num2str(nT),'; ','\epsilon = \epsilon_{*}^{+}','; ',...                  % Если "+"
        'f = var(',num2str(vect_freq(1)),',',num2str(vect_freq(end)),')'])
    
%% Просмотр реализаций
 IND = 2;     % Индекс реализации
 tN = time{IND}/TN;
...
axes2 = axes('Parent',figure2);
    ...
    %yline(-eta_s*cos(72*pi/180),'--k',{'support'},'FontName','Times New Roman','FontSize',14);
    %yline(eta_s,'--k',{'supports'},'FontName','Times New Roman','FontSize',14);
