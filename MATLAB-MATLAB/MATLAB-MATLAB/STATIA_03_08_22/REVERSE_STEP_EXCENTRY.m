...
vect_disbalance = [vect_disbalance(1:29),linspace(vect_disbalance(29),vect_disbalance(1),57)];   % Проверяем обратный ход
le_vect_disbalance = length(vect_disbalance);

tic

%% Получение решения – нужно подгружать решение
index_dis = 29;
x0 = ZZ{1}(end,:);
lenx0 = length(x0);
for j=1:1:lenx0
    XX0(j) = x0(j)+0.1*x0(j)*(rand(1)-1/2); % delta = sqrt(z1^2+z2^2+z3^2+...+z48^2) 
end
[time,ZZZ] = ode23t(@(time,ZZZ) rGap_Multiple_Supports(time,ZZZ,DD,A0,A1,A2,N,eD,mnew,eta_s,kappa_s,nn,D_s,vect_disbalance(index_dis),gd,f_s,NumbOp,ii,System_name),T,XX0,opt);
tN = time/TN;
...
axes2 = axes('Parent',figure2);
    ...
    plot(tN,ZZZ(:,mnew+ii),tN,ZZZ(:,mnew+ii+1))
    ...
    title(['\epsilon = ',num2str(vect_disbalance(index_dis))])
    xlabel(['Displacement, ','{\itt} / (2 \pi / N )'],'FontName','Times New Roman','FontSize',16)
    
    grid on;
    
    subplot(212);hold on;box on
    plot(tN,ZZZ(:,2*(mnew+1)+(mnew)+ii),tN,ZZZ(:,2*(mnew+1)+(mnew+1)+ii))
...
ZxPoincare=interp1(time,ZZZ(:,mnew+ii),tPoincare,'spline');   % Вектор перемещений по oX через каждый оборот 
ZyPoincare=interp1(time,ZZZ(:,mnew+ii+1),tPoincare,'spline'); % Вектор перемещений по oY через каждый оборот
...
figure3 = figure('WindowState','maximized');
 axes3 = axes('Parent',figure3);
    hold on;box on
    plot(-ZZZ(I,mnew+ii+1),ZZZ(I,mnew+ii),'LineWidth',2);
    lenPoin=length(II);
    for j=1:1:lenPoin
        if j<=3
            kColor='o';
    ...
    title([num2str(nT-tNS2),' < nT < ',num2str(nT),'; ','\epsilon = ',num2str(vect_disbalance(index_dis))])
    grid on;
    
%% Коэффициент прецессии
    % Коэффициент прецессии
    [tPr,CoeffPr] = PrecessionCoeff(time(I),ZZZ(I,mnew+ii),ZZZ(I,mnew+ii+1),N);
    tayPI = tPr;
    PI = CoeffPr;
    Average = mean(PI);
    % Спектральный анализ коэффициента прецессии
    PowerSignal = 0.999;
    [fPi,RealizationPi] = spectrum_Fig_Power(tayPI,PI,0,1,PowerSignal);
