...
% Вычисление матриц функций Грина
    for j=1:m+1     % проходим по строкам матриц Грина
        for k=1:m+1 % проходим по столбцам матриц Грина
            [G00Int(j,k),G01Int(j,k),G02Int(j,k),G10Int(j,k),G11Int(j,k),G12Int(j,k)]=MatrixOfGreenIntegrate(j,k,h,alf);
            [G00(j,k),G01(j,k),G02(j,k),G10(j,k),G11(j,k),G12(j,k)]=MatrixOfGreen(j,k,h,alf);
        end
    end
    

    function [G00,G01,G02,G10,G11,G12]=MatrixOfGreenIntegrate(j,k,h,alf)
    % G(r,s)
    r=h*(j-1); 
    s=h*(k-1);
    H=(r>=s);
    
    %% Заделка - подвижная заделка
    C10=-1;         C11=0;      C12=0;
    C20=-(2*s-s^2-2*s*alf)/(2*(alf-1));  C21=-(2-2*s-2*alf)/(2*(alf-1));    C22=1/(alf-1);
    C30=0;          C31=0;      C32=0;
    C40=0;          C41=0;      C42=0;
    ...
    function [G00,G01,G02,G10,G11,G12]=MatrixOfGreen(j,k,h,alf)
    % G(r,s)
    r=h*(j-1); 
    s=h*(k-1);
    H=(r>=s);
