...
N = 18.5;                     % [-] - cкорость вращения ротора
...
vect_gap = (20*10^-3/l);
%vect_gap = linspace((10*10^-3/l),(20*10^-3/l),10);
...
for k = 1:le_vect_gap
    ...
    ff.FontSize = 20;
    xlabel('\chi_{j}');
    ylabel('Экстремумы \xi_{\it x}')
subplot(212)
hold on; box on; grid on;
plot(vect_gap,Average,'.r','MarkerSize',18)
plot(vect_gap,Average)
    ff = gca; 
    ff.FontName = 'Times New Roman';
    ff.FontSize = 20;
    xlabel('\chi_{j}');
...
%% Реакции в упругодемпфирующих опорах
    ...
    ind = 1;
Reactions(time{ind},ZZ{ind},TN,tNS,NumbOp,disp,disp_vel,kappa_j,vect_gap(ind),d_j,f_j,N,eD)
