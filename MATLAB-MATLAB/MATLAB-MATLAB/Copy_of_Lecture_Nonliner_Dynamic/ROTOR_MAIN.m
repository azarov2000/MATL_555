...
m = 30;                     % количество участков разбиения
...
System_name=1;              % (1) "заделка – заделка"; (2) "консоль"
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
nT = 12000;    % количество оборотов ротора
...
Moving_the_center(t,Z0,TN,tNS,disp,angle,N)

%% Траектория центра диска со стробоскопическим отображением
    tNS = 500; % Количство последних оборотов
...
%% Спектральный анализ перемещений
    tNS = 2000; % Количество последних оборотов
...
plot_Disp_3D(t,Z0,TN,tNS,m,index_disp,jC,System_name)

% %% ДОПОЛНЕНИЕ (получение биффуркационной диаграммы и запоминание векторов состояния при различных параметрах)
vect_gap = linspace(chi_j,2*chi_j,10);
le_vect_disbalance = length(vect_gap);
NUMREV=300; % количество последних оборотов
for j=1:1:le_vect_disbalance
     j 
    [t1,Z1]=ode23t(@(t,Z0) rGap_Multiple_Supports(t,Z0,A0,A1,A2,g,h_matr,N,eD,mnew,muR,vect_gap(j),kappa_j,d_j,epsilon,gamma,f_j,NumbOp,disp,disp_vel),T,x0,opt);
    tEta=t1/TN;
    NumerOfRev=find(tEta>(tEta(end)-NUMREV));
    [textrEtaX,XextrEta] = ext(tEta(NumerOfRev),Z1(NumerOfRev,disp(1)));
    [textrEtaY,YextrEta] = ext(tEta(NumerOfRev),Z1(NumerOfRev,disp(2)));
    MaxKSIx{j}=XextrEta;
    MaxKSIy{j}=YextrEta;
...
for k = 1:le_vect_disbalance
    plot(vect_gap(k),MaxKSIx{k},'.r','MarkerSize',18);
end
    ff = gca; 
    ff.FontName = 'Times New Roman';
    ff.FontSize = 20;
    xlabel('\epsilon');
    ylabel('Экстремумы \xi_{x}')
    
% Теперь можно смотреть различные реализации 

%% Графики перемещений и углов поворота
    tNS = 20; % Количство последних оборотов
Moving_the_center(time{1},ZZ{1},TN,nT,tNS,disp,angle,N,chi_j)


%% Траектория центра диска со стробоскопическим отображением
    tNS = 300; % Количство последних оборотов
Phase_trajectory_2D(time{1},ZZ{1},TN,tNS,disp,N)

%% Спектральный анализ перемещений
    tNS = 1000; % Количество последних оборотов
Spectral_analysis(time{1},ZZ{1},tNS,TN,disp)


%% Коэффициент прецессии
    tNS = 300; % Количство последних оборотов
Precession_coefficient(time{1},ZZ{1},TN,tNS,disp,N)
%%
    tNS = 300;
Reactions(time{1},ZZ{1},TN,tNS,NumbOp,disp,disp_vel,kappa_j,chi_j,d_j,f_j,N,eD)
