...
%============================================
% Настройка решателя
opt=odeset('AbsTol',1e-5,'RelTol',1e-5);
...
%=========================================
e1=linspace(0,0.04,30);
le=length(e1);
LeTNT=length(III);
YYY=zeros(LeTNT,le);
for ee=1:le
[te,Ye]=ode45(@(t1,Y1) rGap1(t1,Y1,e1(ee), eta_s),TNT,x0,opt);
YYY(:,ee)=Ye(III,1);
end
% Точки Пуанкаре от изменения эксцентриситета
 f10=figure;        % xi_y VS xu_x       
    hold on;box on
    for j=1:le
     plot(e1(j),YYY(:,j),col,'MarkerSize',ms)
    end
    xlabel('\epsilon')
    ylabel('\xi_{ x}')
    axis equal
...
% Экстремумы графиков от изменения эксцентриситета



%=========================================
    eta1=linspace(0,0.05,30);
leeta=length(eta1);
LeTNT=length(III);
YYYeta=zeros(LeTNT,leeta);
for eta=1:leeta
[teta,Yeta]=ode45(@(t1,Y1) rGap1(t1,Y1,e,eta1(eta)),TNT,x0,opt);
YYYeta(:,eta)=Yeta(III,1);
end
% Точки Пуанкаре от изменения зазора
 f11=figure;        % xi_y VS xu_x       
    hold on;box on
    for j=1:leeta
     plot(eta1(j),YYYeta(:,j),col,'MarkerSize',ms)
    end
    xlabel('\eta_{ s}')
    ylabel('\xi_{ x}')
    axis equal
