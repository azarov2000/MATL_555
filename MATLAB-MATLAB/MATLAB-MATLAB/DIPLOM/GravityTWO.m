...
[t2,Y2]=ode45(@(t2,Y2) rGap2(t2,Y2),T,x0,opt);
tN2=t2/TN;
%============================================
...
tNS=50;
 ...
 I2=find(tN2>(tN2(end)-tNS));% tN>1000-50, т.е. берутся элементы 950-1000

%============================================
% График траектории ценра диска
 figure;        % xi_y VS xu_x       
    hold on;box on
    plot(Y(I,2),Y(I,1),Y2(I2,2),Y2(I2,1))
    axis equal
...
%============================================
% Функция формирования дифферециальных уравнений с учетом тяжести
function S=rGap(t,Y)
...
% Функция формирования дифферециальных уравнений без учета тяжести
function S2=rGap2(t2,Y2)
    
        c2=[cos(N*t2);sin(N*t2)]; % учёт эксцентриситета
        P2=-e*N*(N*ee-2*Zeta_e*SS)*c2;

        Vrel=(N*eD/2-Y2(6))/abs(N*eD/2-Y2(6)); % учет трения об опору
        C2=[1;f_s*Vrel];
        fh2=-((Y2(1)-eta_s)>=0)*(kappa_s*(Y2(1)-eta_s)^nn+D_s*Y2(5));
        F2=fh2*C2;

    ff=A2\[G00*(P2+F2);-Gr0*SS*(P2+F2)]; % вектор G 
    a=Y2(1:4);b=Y2(5:8);
    S2=[b;-B1*b-B0*a+ff];
end
end
