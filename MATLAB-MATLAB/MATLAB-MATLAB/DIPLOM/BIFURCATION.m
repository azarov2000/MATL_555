...
pause(5)
 ...
 kappa_s=50000;        % Жесткость ограничителя
...
tN=t/TN;
TNT=(0:TN:tlim);
[t1,Y1]=ode45(@(t1,Y1) rGap1(t1,Y1),TNT,x0,opt);
TNN=t1/TN;
%============================================
...
figure;               
    ...
    plot(Y(I,2),Y(I,1),Y1(II,2),Y1(II,1),'r.','MarkerSize',10) 
    %plot(Y1(II,2),Y(II,1),'c*')
    axis equal
    xlabel('\xi_{ y}')
...
%============================================
% График сил от действия упругой опоры на колечного промежутке
figure;         % F_x VS t/TN
    subplot(211);hold on;box on;grid on
    plot(tN(I),F(1,I))
    ylabel('F_{ s x}')
...
function S=rGap(t,Y)
    ...
    ff=A2\[G00*(P+F);-Gr0*SS*(P+F)]; % вектор G 
    a=Y(1:4);b=Y(5:8);
    S=[b;-B1*b-B0*a+ff];
end
function S1=rGap1(t1,Y1)
    
        c1=[cos(N*t1);sin(N*t1)]; % учёт эксцентриситета
        P1=e*N*(N)*c1;
    ...
    ff1=A2\[G00*(P1+F1);-Gr0*SS*(P1+F1)]; % вектор G 
    a1=Y1(1:4);b1=Y1(5:8);
    S1=[b1;-B1*b1-B0*a1+ff1];
end
end
