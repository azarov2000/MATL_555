...
vect_disbalance = linspace(0,e,15);
...
% Фазовая траектория и стробоскопическое отображение
...
Poincare_x = ZxPoincare(II).*cos(alfa_Poin)+ZxPoincare(II).*sin(alfa_Poin);
Poincare_y = -ZxPoincare(II).*sin(alfa_Poin)+ZxPoincare(II).*cos(alfa_Poin);

% Спектральнный анализ перемещений
tt=(tN(I)*TN);
Zx=(ZZ{IND}(I,mnew+ii))';
...
[extr_t,extr_lambda] = ext(tayPI{IND},PI{IND});
le_extr_lambda = length(extr_lambda); 
vect_x = extr_lambda(1:(le_extr_lambda-1));
vect_y = extr_lambda(2:le_extr_lambda);

% Квазифазовая траектория для перемещений
le_MAXKSIx = length(MaxKSIx{IND}); 
vect_x_disp = MaxKSIx{IND}(1:(le_MAXKSIx-1));
vect_y_disp = MaxKSIx{IND}(2:le_MAXKSIx);

%%                                                        Построение биффуркационной диаграммы и средних значений Лямбда
 ...
 ff=get(gca);
 plot(ff.XLim,[0,0],'k-',ff.XLim,-[eta_s,eta_s],'k-')
 %plot(ff.XLim,[0,0],'k-',ff.XLim,-[eta_s,eta_s],'k-')               % Одна опора 
 %plot(ff.XLim,[0,0],'k-',ff.XLim,-[eta_s,eta_s],'k-',ff.XLim,[eta_s,eta_s],'k-')               % Одна опора 
...
ff=get(gca);
    ...
    title([num2str(nT-NUMREV),' < nT < ',num2str(nT),'; ','\epsilon = ',num2str(vect_disbalance(IND))])
     ...
     subplot(2,1,2)
     h1=stem(fPI{IND},AmplitudePI{IND});
...
axes2 = axes('Parent',figure2);
   ...
   % yline(-eta_s,'--k',{'\bfsupport № 1'},'LabelHorizontalAlignment','center','FontName','Times New Roman','FontSize',14);
    yline(eta_s*0.5,'--k',{'\bfsupports № 1,3'},'FontName','Times New Roman','FontSize',14);
...
figure3 = figure('WindowState','maximized');
 axes3 = axes('Parent',figure3);
    ...
    axis equal
    xlabel('\xi_{\ity}')
    ylabel('\xi_{\itx}','Rotation',0)
    title([num2str(nT-tNS2),' < nT < ',num2str(nT),'; ','\epsilon = ',num2str(vect_disbalance(IND))])
    ff = gca; 
    ff.FontName = ' Times New Roman';
    ff.FontSize = 20;
    grid on;
%% Квазифазовая траектория во вращающейся системе
figure;
...
ff=get(gca);
     ...
     ff.FontName = ' Times New Roman';
     ff.FontSize = 20;
             %% Построение квазифазовой траектории для перемещений
figure;
grid on; hold on; box on;
        p = plot(vect_x_disp(:),vect_y_disp(:),'.');
        p.MarkerSize = 20;
        p.MarkerFaceColor = 'k';
        x = [-0.1 0.1];
        y = [-0.1,0.1];
