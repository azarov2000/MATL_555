...
kappa_s = 850 *(3/5);          % коэффициент жесткости ограничителей
...
nT=8000;      % число периодов - нужно много периодов для получения установившегося решения
...
Grav_beam =[kron(G00Int,E) , zeros(2*(mnew+1) , 2*(mnew+1)) ; zeros(2*(mnew+1),2*(mnew+1)) , kron(G10Int,E)]; %!!!!!!!!!!!!!!!!!!
 %% Изменение нумирации
...
for j=1:1:le_vect_disbalance
    ...
    j
    [t,Z0]=ode23t(@(t,Z0) rGap_Multiple_Supports(t,Z0,DD,A0,A1,A2,N,eD,mnew,eta_s,kappa_s,nn,D_s,vect_disbalance(j),gd,f_s,NumbOp,ii,System_name,muR,Grav_beam),T,x0,opt);
    tEta=t/TN;
...
%% Построение биффуркационной диаграммы
 ...
 %plot(ff.XLim,[0,0],'k-',ff.XLim,-[eta_s,eta_s]-(gd/3),'k-',ff.XLim,[eta_s,eta_s]-(gd/3),'k-')               % Одна опора 
...
figure3 = figure('WindowState','maximized');
 axes3 = axes('Parent',figure3);
    ...
    %viscircles([0,0],0.02,'Color','black','LineStyle','--');
    axis equal
    xlabel('\xi_{\ity}')
...
plot(-Poincare_y,Poincare_x,'.r','MarkerSize',12)
ff = gca;
ff.FontName = ' Times New Roman';
ff.FontSize = 20;
title([num2str(nT-tNS2),' < nT < ',num2str(nT),'; ','\epsilon = ',num2str(vect_disbalance(IND))])
...
lenI = length(I);
clear z_disp x_disp y_disp
    x_disp = [zeros(length(I),1),ZZ{1}(I(:),1:2:11)];
    y_disp = [zeros(length(I),1),ZZ{1}(I(:),2:2:12)];
    for j=1:length(I)
