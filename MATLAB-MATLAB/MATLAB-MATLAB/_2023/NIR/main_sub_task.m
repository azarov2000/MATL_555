...
a_theta_0 = Mom*kron(G01*EC,E)-Mom*kron(G02Int,E);

b_ksi_0 = -kappa_B*kron(G10*EC,S);
b_theta_0 = kron(I,(E+2*zeta_V*N*S))-Mom*kron(G11*EC,S)+Mom*kron(G12Int,S);

A0 = [a_ksi_0, a_theta_0;
      b_ksi_0, b_theta_0];

a_ksi_1 = 2*zeta_V*kron(I,E)+2*zeta_e*kron(G00Int,E)+2*eta_B*kron(G00*EC,E);
a_theta_1 = 2*beta*N*kron(G00*EC,E)-2*beta*N*kron(G01Int,E);

b_ksi_1 = -2*zeta_e*kron(G10Int,S)-2*eta_B*kron(G10*EC,S);
b_theta_1 = 2*zeta_V*kron(I,E)-2*beta*N*kron(G10*EC,S)+2*beta*N*kron(G11Int,S);

A1 = [a_ksi_1, a_theta_1;
      b_ksi_1, b_theta_1];

a_ksi_2 = kron(G00Int,E)+mu_B*kron(G00*EC,E);
a_theta_2 = -beta*kron(G00*EC,S)+beta*kron(G01Int,S);

b_ksi_2 = -kron(G10Int,S)-mu_B*kron(G10*EC,S);
b_theta_2 = -beta*kron(G10*EC,E)+beta*kron(G11Int,E);
...
for j=1:length(M_v)
    ...
    for i=1:length(zetaV_v)
        ...
        temp_data.Mom = M_v(j);
        N_critical_zetaV{j}(i) = Argan_diagram(temp_data,N_start,step_N);
        temp_data = data;
    end
end
%%
figure;
box on; grid on; hold on; 
plot(zetaV_v,N_critical_zetaV{1},'.b','MarkerSize',20)
plot(zetaV_v,N_critical_zetaV{1},'-b','LineWidth',2)
plot(zetaV_v,N_critical_zetaV{2},'.r','MarkerSize',20)
plot(zetaV_v,N_critical_zetaV{2},'-r','LineWidth',2)
plot(zetaV_v,N_critical_zetaV{3},'.k','MarkerSize',20)
plot(zetaV_v,N_critical_zetaV{3},'-k','LineWidth',2)
plot(zetaV_v,N_critical_zetaV{4},'.g','MarkerSize',20)
plot(zetaV_v,N_critical_zetaV{4},'-g','LineWidth',2)
ff = gca;
