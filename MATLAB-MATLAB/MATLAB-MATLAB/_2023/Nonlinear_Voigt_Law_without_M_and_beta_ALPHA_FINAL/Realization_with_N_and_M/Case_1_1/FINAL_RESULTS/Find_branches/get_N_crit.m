function [Ncritical,complex_indicator] = get_N_crit (dN,step_N,d)
  ...
  while (flag<=1)
      ...
      if max(real(EigenValues{i})>0)
          flag = flag+1;
          %%%
          index_max = find(real(EigenValues{i}) == max(real(EigenValues{i})));
          complex_indicator = imag(EigenValues{i}(index_max));
          %%%
      end
