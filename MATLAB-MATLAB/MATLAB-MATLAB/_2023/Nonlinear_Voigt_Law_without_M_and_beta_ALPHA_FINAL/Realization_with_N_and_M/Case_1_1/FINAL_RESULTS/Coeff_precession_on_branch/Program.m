...
% Find N_critical
[mc.Ncritical,mc.freq_biff] = get_N_crit (0,0.1,mc);

%% Обратный ход (верхняя ветвь)
tmp_mc = mc;

N_vector_reverce = 31.65:-0.2:10;
T = [0,6000];
...
for i=1:length(RES_N_rev)
    plot(N_vector_reverce(i),RES_N_rev{i}.CoffPrecession,'.r','MarkerSize',15);
end
ff = gca;
ff.FontSize = 16;
xlabel('\Omega')
