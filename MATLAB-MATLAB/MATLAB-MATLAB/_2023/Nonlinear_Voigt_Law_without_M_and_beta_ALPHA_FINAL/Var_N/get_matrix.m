function [G00Int,G02Int,G00,G02]=get_matrix(m)
...
% Вычисление матриц функций
    for j=1:m+1     % проходим по строкам матриц
        for k=1:m+1 % проходим по столбцам матриц
            [G00Int(j,k),G02Int(j,k)]=MatrixOfGreenIntegrate(j,k,h);
            [G00(j,k),G02(j,k)]=MatrixOfGreen(j,k,h);
        end
    end
    
    function [G00,G02]=MatrixOfGreenIntegrate(j,k,h)
    ...
    function [G00,G02]=MatrixOfGreen(j,k,h)
