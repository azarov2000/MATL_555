...
% Вычисление матриц функций
    for j=1:m+1     % проходим по строкам матриц
        for k=1:m+1 % проходим по столбцам матриц
            [G00Int(j,k),G01Int(j,k),G02Int(j,k),G20Int(j,k),G21Int(j,k)]=MatrixOfGreenIntegrate(j,k,h);
            [G00(j,k),G01(j,k),G02(j,k),G20(j,k),G21(j,k)]=MatrixOfGreen(j,k,h);
            [J(j,k)]=MatrixJ(j,k,h);
        end
    end
    
    function [G00,G01,G02,G20,G21]=MatrixOfGreenIntegrate(j,k,h)
    ...
    function [G00,G01,G02,G20,G21]=MatrixOfGreen(j,k,h)
