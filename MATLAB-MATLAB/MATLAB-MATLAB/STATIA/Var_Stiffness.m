...
tic
eta_s=(7.5*10^-3)/0.7;
vect_stiff=[linspace(420,470,10),linspace(470,800,20)];
vect_stiff=linspace(vect_stiff(13),vect_stiff(15),5);
vect_stiff=linspace(vect_stiff(2),vect_stiff(4),11);
%%
...
axis([vect_stiff(1)-mean(vect_stiff(1),vect_stiff(end))*0.01,vect_stiff(end)+mean(vect_stiff(1),vect_stiff(end))*0.01,-0.2,0.2]);
...
axes2 = axes('Parent',figure2);
    ...
    grid on;
    title(['При ','\kappa = ',num2str(vect_stiff(3))],'FontName','Times New Roman','FontSize',16)    
    subplot(212);hold on;box on
    plot(tN,Z0(:,2*(mnew+1)+(mnew)+ii),tN,Z0(:,2*(mnew+1)+(mnew+1)+ii))
    legend('\vartheta_{ x}','\vartheta_{ y}')
    xlabel(['Углы поворота, ','{\itt} / (2 \pi / N )'])
%% 
...
%%
 ...
 axes3 = axes('Parent',figure3);
    hold on;box on
    plot(-Z0(I,mnew+ii+1),Z0(I,mnew+ii),-ZyPoincare(II),ZxPoincare(II),'r.','MarkerSize',18)
    %plot(-Z0(I,mnew+ii+1),Z0(I,mnew+ii))
    axis equal
    xlabel('\xi_{ y}')
    ylabel('\xi_{ x}','Rotation',0)
    title(['\xi_{ y} VS \xi_{ x}',';  ',num2str(nT-tNS),' < nT < ',num2str(nT)],'FontName','Times New Roman','FontSize',20)
...
%%
%%
[fx,Xspectr]=spectrum_Fig_Power(ttInt,ZZx,0,1,0.95);
[fy,Yspectr]=spectrum_Fig_Power(ttInt,ZZy,0,1,0.95);
%%
...
ff=get(gca);
     ...
     title('Перемещения вдоль оси оX','FontName','Times New Roman','FontSize',20)
     grid on;
     
     subplot(2,2,2)
     h1=stem(fx,Xspectr);
     xline(1/TN,'--r',{'Rotation speed'},'FontName','Times New Roman','FontSize',14);
     xlabel('f','FontName','Times New Roman','FontSize',16)
     ylabel('Amplitude','FontName','Times New Roman','FontSize',16)
     title('Спектр по оX','FontName','Times New Roman','FontSize',20)
     grid on;
     ...
     title('Перемещения вдоль оси оY','FontName','Times New Roman','FontSize',20)
     grid on;
     ...
     xlabel('f','FontName','Times New Roman','FontSize',16)
     ylabel('Amplitude','FontName','Times New Roman','FontSize',16)
     title('Спектр по оY','FontName','Times New Roman','FontSize',20)
     grid on;
