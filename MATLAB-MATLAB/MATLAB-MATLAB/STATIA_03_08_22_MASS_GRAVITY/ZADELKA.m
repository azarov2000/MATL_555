...
figure4 = figure('WindowState','maximized');
     ...
     %title([num2str(nT-tNS2),' < nT < ',num2str(nT),'; ','N = ', num2str(N),'; ','\chi = ',num2str(eta_s)])
...
figure2 = figure('WindowState','maximized');
    ...
    grid on;    
    %title(['N = ',num2str(N),'; ','\chi = ', num2str(eta_s)])
    title(['N = ',num2str(N)])
    ax2 = gca;
    ...
    plot(tN,Z0(:,2*(mnew+1)+(mnew)+ii+1),tN,Z0(:,2*(mnew+1)+(mnew+1)+ii+2))
