...
eta_s = (7.5*10^-3)/0.7;  % коэффициент зазора
...
x0=0.01*[x0;zeros(2*(mnew+1),1);zeros(4*(mnew+1),1)];
...
plot(vect_N(:)/(2*pi),AveragePi(:),'-r')
xline(14.2624/(2*pi),'--',{'N_{critical}'},'FontName','Times New Roman','FontSize',16,'LabelVerticalAlignment','middle')
str = '$$ \overline{\Lambda} $$';
xlabel('N, 1/\tau')
ylabel(str,'interpreter','latex')
title([num2str(nT-tNS),' < nT < ',num2str(nT)])
set(axes2,'FontName','Times New Roman','FontSize',18)
