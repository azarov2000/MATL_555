...
Ncritical = Ncritical-0.3439;
NInterp = NInterp-0.3439;
%% Nkrit VS etai
...
hold(axes1,'on');grid on; box on;
    for j=1:lenzi
        plot(zi(j),Ncritical(j),'r.','MarkerSize',30)
    end
        plot(ziInterp,NInterp,'-r');

xlabel('\eta_{ V}','FontName','Times New Roman','FontSize',30)
ylabel('N_{ critical} ','FontName','Times New Roman','FontSize',30)
title('The effect of internal friction on the critical speed of rotation','FontName','Times New Roman','FontSize',30)
