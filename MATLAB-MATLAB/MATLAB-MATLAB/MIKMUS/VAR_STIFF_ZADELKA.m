...
tic
%--------------------------------------------------------------------------
N=18.5;               % Скорость вращения ротора
m=6;                % Количество участков
NumbOp=60:120:300;  % Расположение опор (degrees)

zDisk=0.5;            % Кооридната расположения диска
...
for j=1:1:le_vect_stiff
    ...
    [tPr,CoeffPr] = PrecessionCoeff(tay{j},Z0(I,mnew+ii+1),Z0(I,mnew+ii+2),N);
    tayPI{j} = tPr;
    PI{j} = CoeffPr;
    Average(j) = mean(PI{j});
...
ff=get(gca);
     ...
     title([num2str(nT-NUMREV),' < nT < ',num2str(nT),'; ',...                  % Если "+"
        '\kappa = ',num2str(vect_stiff(indexPressigion))])
     grid on;
     ff = gca; 
...
figure3 = figure('WindowState','maximized');
 axes3 = axes('Parent',figure3);
    hold on;box on
    plot(-ZZ{IND}(I,mnew+ii+2),ZZ{IND}(I,mnew+ii+1),'color',"#D95319",'LineWidth',2);
    lenPoin=length(II);
    for j=1:1:lenPoin
        if j<=3
            kColor='o';
...
%% Зависимость Average от значения параметра
    ...
    ff. FontSize = 20;
    grid on;
    xlimits1 = (vect_stiff(1)-(vect_stiff(end)-vect_stiff(1))/20);
    xlimits2 = (vect_stiff(end)+(vect_stiff(end)-vect_stiff(1))/20);
