...
timePI = TN*tN(I);
[tPr,CoeffPr] = PrecessionCoeff(timePI,Z0(I,mnew+ii),Z0(I,mnew+ii+1),N);
tPI = tPr;
PI = CoeffPr;
PowerSignal = 0.999;
...
figure3 = figure('WindowState','maximized'); 
    hold on;box on
    plot(-Z0(I,mnew+ii+1),Z0(I,mnew+ii),'LineWidth',0.5);
        plot(-Zy1,Zx1,'LineWidth',0.5); % удаление
    lenPoin=length(II);
    for j=1:1:lenPoin
        if j<=3
            kColor='o';
            ...
            p=plot(-ZyPoincare(II(j)),ZxPoincare(II(j)),kColor,'MarkerSize',kSize);
                p1 = plot(-ZyPoincare1(j),ZxPoincare1(j),kColor,'MarkerSize',kSize); % удаление
            ...
            plot(-ZyPoincare(II(j)),ZxPoincare(II(j)),kColor,'MarkerSize',kSize);
                plot(-ZyPoincare1(j),ZxPoincare1(j),'b.','MarkerSize',kSize);
        end
    end
    legend('\xi_{ x}','\xi_{ y}','FontName','Times New Roman','FontSize',20)
